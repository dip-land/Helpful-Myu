...
function finish(message: Message<boolean>, channel: MessageFilterConfigInterface, noReset?: boolean) {
    ...
    for (const emoji of channel.emojis) {
        message.react(emoji).catch((err: Error) => {
            if (err.message === 'Reaction blocked') return console.log(client.timeCode('error'), `${message.author.username} has the bot blocked.`);
            return console.log(client.timeCode('error'), 'error reacting to a message', err);
        });
    }
}
