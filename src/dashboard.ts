...
function createBot(version: string): void {
    ...
    const _version = version ? version.replace('-', '') : 'beta';
    bot.stdout.on('data', (data) => {
        console.log(data.toString().replace('\n', ''));
        ProcessOut.push(`${_version} >${data.toString().replace('\n', '').split(']').slice(2).join(' ').replace('\x1b[0m', '').slice(0, 500)}`);
    });
    bot.on('error', (err) => {
        exit(err, _version, version, bot);
        ProcessOut.push(`${_version} > ${err.toString().slice(0, 500)}...`);
    });
    bot.on('exit', (a) => {
        exit(a, _version, version, bot);
        ProcessOut.push(`${_version} > Exited with code ${a}, and is now attempting to restart...`);
    });
    bot.on('spawn', (a: unknown) => {
        console.log(timeCode(_version), 'spawn', bot.pid);
        ProcessOut.push(`${_version} > ${_version} starting up with pid ${bot.pid}`);
    });
}

function exit(type: Error | number | null, _version: string, version: string, bot: ChildProcessWithoutNullStreams): void {
    const isExit = typeof type === 'number' ? true : false;
    console.log(timeCode(_version, 'error'), isExit ? 'exit' : 'error', type);
    ProcessOut.push(`${_version} > Restarting...`);
    if (bot.killed || isExit) {
        createBot(version);
        console.log(timeCode(_version, 'error'), 'bot encontered an error and was reloaded as pid', bot.pid);
    }
}
